...
// Private includes
#include "scan_tools/laser_scan_assembler.hpp"
...
LaserScanAssembler::CloudDescriptor<PointType>::CloudDescriptor(const rclcpp::Time &refresh_timestamp) :
    refresh_stamp{ refresh_timestamp }    
{ }

/* ========================================================= Ctors & dtors ======================================================== */

LaserScanAssembler::LaserScanAssembler(const rclcpp::NodeOptions & options) : 
    rclcpp::Node("laser_scan_assembler", options)
{
    /* ------------------------ Register parameters ------------------------ */
    ...
    for(unsigned i = 0; i < input_topics->size(); ++i) {
        *node_common::communication::make_subscriber_builder(cloud_subs[i])
            ...
            .callback(*this, &LaserScanAssembler::cloud_callback);
    }
...
LaserScanAssembler::~LaserScanAssembler() {
    node_common::node::print_goodbye(*this);
}

/* =========================================================== Callbacks ========================================================== */

void LaserScanAssembler::cloud_callback(const sensor_msgs::msg::PointCloud2 &cloud_msg) {
...
void LaserScanAssembler::reset_cloud_storage(PointTypeId new_point_type) {
...
template<typename PointType>
bool LaserScanAssembler::store_incoming_cloud(
...
template<typename PointType>
std::vector<const LaserScanAssembler::CloudType<PointType>*> 
LaserScanAssembler::select_clouds_to_compile(const rclcpp::Time &now) {
...
template<typename PointType>
void LaserScanAssembler::compile_clouds(
    const rclcpp::Time &now,
    const std::vector<const CloudType<PointType>*> &cloud_components
) {
    
    /**
     * @see https://github.com/stereolabs/zed-ros-wrapper/issues/393
     */
...
RCLCPP_COMPONENTS_REGISTER_NODE(scan_tools::LaserScanAssembler)
