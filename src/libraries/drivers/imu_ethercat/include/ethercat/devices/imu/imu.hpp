 ...
 * @modified   Monday, 13th June 2022 6:24:56 pm
...
Imu<SlaveImplementationT>::Imu(SlaveInterface &slave) :
    ...
    accelerometer_offset_x_sdo      { slave.template get_sdo<decltype(accelerometer_offset_x_sdo     )>(imu::registers::ACCELEROMETER_OFFSET_X.index      ) },
    accelerometer_offset_y_sdo      { slave.template get_sdo<decltype(accelerometer_offset_y_sdo     )>(imu::registers::ACCELEROMETER_OFFSET_Y.index      ) },
    accelerometer_offset_z_sdo      { slave.template get_sdo<decltype(accelerometer_offset_z_sdo     )>(imu::registers::ACCELEROMETER_OFFSET_Z.index      ) },
...
{ 
    /**
     * @note Default configuration of gyro range and gitial filter is set to:
     * 
     *      * "Digital Filter Settings" : N = 2^2 ( config: @c 2 )
     *      * "Gyro Range Settings"     : +/- 300 [°/s] ( config: @c 4 )
     * 
     *   These are default settings of the ADIS16362 IC ( @b SENS_AVG register ).
     * 
