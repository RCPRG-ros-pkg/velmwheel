                         ...
                         uint32_t ulFileSize, PFN_RECV_PKT_CALLBACK pfnRecvPktCallback, void* pvUser)
{
  ...
  if( eCIFX_DEVICE_FLASH_BASED == ptDevInstance->eDeviceType)
  {
    ...
    ulCopySize  = min_macro( (sizeof(uSendPacket.tPacket.abData) - sizeof(uSendPacket.tLoadAndRunReq.tData)), ulNameLength);
  ...
  } else
  {
    ...
    ulCopySize  = min_macro( (sizeof(uSendPacket.tPacket.abData) - sizeof(uSendPacket.tRunReq.tData)), ulNameLength);
