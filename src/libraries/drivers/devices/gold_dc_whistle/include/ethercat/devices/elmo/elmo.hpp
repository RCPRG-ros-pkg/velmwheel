 ...
 * @modified   Wednesday, 29th June 2022 11:14:52 pm
...
Elmo<SlaveImplementationT>::Elmo(SlaveInterface &slave, const Config &rconfig) :
    ...
    config { 
        ...
        .motor_rate_torque  { rconfig.motor_rate_torque                                              },
        .motor_rate_current { rconfig.motor_rate_current                                             }
    },

    // SDO interfaces
    mode_of_operation_sdo { slave.template get_sdo<decltype(mode_of_operation_sdo)>(elmo::registers::MODES_OF_OPERATION.index ) },
    polarity_sdo          { slave.template get_sdo<decltype(polarity_sdo)>         (elmo::registers::POLARITY.index           ) },
    // PDO entry references
    status_word_pdo  { slave.template get_pdo_entry<PdoDirection::Input> (elmo::registers::STATUSWORD.name ).template get_reference<decltype(status_word_pdo)>()  },
    control_word_pdo { slave.template get_pdo_entry<PdoDirection::Output>(elmo::registers::CONTROLWORD.name).template get_reference<decltype(control_word_pdo)>() }
